...
def ods_content(row, h, u, p, d, t, m):
    ...
    with open(file_path+'/'+d+'/'+'/'+t+'/ods_'+d+'_'+t+'_'+m+'_d.sh', 'w') as f:
        ...
        f.write("  --connect jdbc:mysql://%s:3306/%s?tinyInt1isBit=false " %(h,d))
        f.write("  --username %s  " %u)
        f.write("  --password %s  " %p)
        f.write("  --query \"$sql\" ")
        f.write("  --target-dir /user/hive/warehouse/ods.db/ods_%s_%s_%s_d/dt=${p_date}/dc=${datacenter}/tz=${timezone}  " %(d,t,m)) 
